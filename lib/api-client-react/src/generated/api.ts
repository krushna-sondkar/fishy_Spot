 ...
 */
export const getListFishUrl = () => {
  return `/api/fish`;
};

export const listFish = async (options?: RequestInit): Promise<Fish[]> => {
  return customFetch<Fish[]>(getListFishUrl(), {
    ...options,
    method: "GET",
  });
};

export const getListFishQueryKey = () => {
  return [`/api/fish`] as const;
};

export const getListFishQueryOptions = <
  TData = Awaited<ReturnType<typeof listFish>>,
  TError = ErrorType<unknown>,
>(options?: {
  query?: UseQueryOptions<Awaited<ReturnType<typeof listFish>>, TError, TData>;
  request?: SecondParameter<typeof customFetch>;
}) => {
  const { query: queryOptions, request: requestOptions } = options ?? {};

  const queryKey = queryOptions?.queryKey ?? getListFishQueryKey();

  const queryFn: QueryFunction<Awaited<ReturnType<typeof listFish>>> = ({
    signal,
  }) => listFish({ signal, ...requestOptions });

  return { queryKey, queryFn, ...queryOptions } as UseQueryOptions<
    Awaited<ReturnType<typeof listFish>>,
    TError,
    TData
  > & { queryKey: QueryKey };
};

export type ListFishQueryResult = NonNullable<
  Awaited<ReturnType<typeof listFish>>
>;
export type ListFishQueryError = ErrorType<unknown>;
...
>(options?: {
  ...
  const queryOptions = getListFishQueryOptions(options);
